...
2. Ejecuta calcAvgVector() para promediar:
   ...
   - Todas las normas
3. Guarda el resultado en archivos:
...
```
classify() comienza cargando desde src/dataset/:
```
...
- mic_norm = carga src/dataset/micProcessed.txt (generado en PASO 7)
- mic_acov = carga src/dataset/micAcov.txt (generado en PASO 7)
```
...
8. [src/utils/audioTransformer.py.detener()] → Promedia todos los
   fragmentos y guarda en src/dataset/
   ↓
9. [src/models/classifier.py] → Carga vectores de referencia desde
   src/dataset/
   ↓
...
│   │   ├── wnAcov.txt               ← Autocovariance WN de referencia
│   │   ├── micProcessed.txt         ← Audio del micrófono procesado ← GENERADO PASO 7
│   │   └── micAcov.txt              ← Autocovariance del micrófono ← GENERADO PASO 7
│   │
│   └── loaders/
...
5. Usuario detiene (PASO 7)
6. Se clasifica (PASOS 8-11)
7. Se muestra resultado (PASO 12-13)
```
